      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-2xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden border border-slate-200 dark:border-slate-800">
        <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-8 py-6 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-sm mt-1 text-slate-900 dark:text-slate-400">Cargá las actividades y observaciones del día.</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl border border-slate-300 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-900 dark:text-slate-400 transition-colors">✕</button>
        </div>
        <div className="p-8 overflow-y-auto flex-1 custom-scrollbar">
          <form id="sesionForm" onSubmit={onSubmit} className="space-y-6">
            <div>
              <label className="block mb-2 font-semibold text-slate-900 dark:text-slate-400">Fecha de la Sesión</label>
            ...
            <div>
              <label className="block mb-2 font-semibold text-slate-900 dark:text-slate-400">Actividades Realizadas</label>
            ...
            <div>
              <label className="block mb-2 font-semibold text-slate-900 dark:text-slate-400">Observaciones / Evolución</label>
        ...
        <div className="border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-8 py-5 flex justify-end gap-4 shrink-0">
          <button type="button" onClick={onClose} className="px-6 py-2.5 font-bold rounded-xl transition-colors text-slate-900 hover:bg-slate-200 dark:text-slate-300 dark:hover:bg-slate-800">Cancelar</button>
