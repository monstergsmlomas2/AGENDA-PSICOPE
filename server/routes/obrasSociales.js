...
router.get("/", async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      LEFT JOIN pacientes p ON LOWER(p.obra_social) = LOWER(os.nombre) AND p.usuario_id = $1
      WHERE os.usuario_id = $1
      GROUP BY os.id
  ...
  try {
    const result = await pool.query(
      `SELECT os.*,
        COUNT(p.id) AS cantidad_pacientes
       FROM obras_sociales os
       LEFT JOIN pacientes p ON LOWER(p.obra_social) = LOWER(os.nombre) AND p.usuario_id = $2
       WHERE os.id = $1 AND os.usuario_id = $2
       GROUP BY os.id`,
      [id, req.userId]
    );
...
router.post("/", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO obras_sociales (nombre, codigo, sesiones_autorizadas, valor_sesion, periodo_renovacion, observaciones, usuario_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
      [nombre, codigo, sesiones_autorizadas || 4, valor_sesion, periodo_renovacion || 'mensual', observaciones, req.userId]
    );
    res.json(result.rows[0]);
  } catch (error) {
...
router.put("/:id", async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [nombre, codigo, sesiones_autorizadas, valor_sesion, periodo_renovacion, observaciones, id, req.userId]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Obra social no encontrada" });
    }
...
router.delete("/:id", async (req, res) => {
  const { id } = req.params;
  try {
    const result = await pool.query("DELETE FROM obras_sociales WHERE id = $1 AND usuario_id = $2 RETURNING id", [id, req.userId]);
